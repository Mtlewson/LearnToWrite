...
#writer.save()
filename = 'VisibleDecisionWindows.png'
...
img = np.array(Image.open("VisibleDecisionWindows.png"))
print(255== img[0][0])
...
for x in range(0, xs):
    for y in range(0, ys):
        pixC = img[y, x]
        if np.any(pixC > 5): #if a pixel is not black/near black make it true white
            img[y,x] = 255
...
pixArray = np.array(pixLoc)
csvWriter("VisibleDecisionWindowsExcelv2", pixArray)
#
