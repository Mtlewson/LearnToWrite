...
async def run_training(client):
    ...
    while(True):
        ...
        if direction == b"w": # Forward
            ...
            print(user_command)
            temp2 = bytearray(user_command)
            print(temp2)
            await client.write_gatt_char(UUID_NORDIC_TX, bytearray(user_command), True)
        ...
        try: # Attempt to connect to ESP32
            ...
            async with BleakClient(address, loop=loop) as client:
                while True: # Main loop, user selects test case
                    print("you did it, type q to quit or anything else to move on ")
                    temp = input("")
                    if temp == "q":
                        break
                    await run_training(client)
                    # print("Testing direction, type w, a, s, d")
                    # direction = msvcrt.getche()
                    # intensity = 2
                    # if direction == b"w": # Forward
                    #     print("FORWARD")
                    #     user_command = b"1," + intensity
                    #     await client.write_gatt_char(UUID_NORDIC_TX, bytes(bytearray(user_command[0:20])), True)
                    # elif direction == b"a": # Left
                    #     print("LEFT")
                    #     user_command = b"2," + intensity
                    #     await client.write_gatt_char(UUID_NORDIC_TX, bytearray(user_command[0:20]), True)
